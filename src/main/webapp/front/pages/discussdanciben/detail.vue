...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussdancibenCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussdancibenCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussdanciben');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussdanciben');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussdanciben', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussdanciben', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussdanciben/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussdanciben', this.detail);
				this.$utils.msg('审核成功');
