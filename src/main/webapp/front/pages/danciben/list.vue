<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.dancimingcheng" type="text" placeholder="单词名称" ></input>
			</view>
                  			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.dancifenlei" type="text" placeholder="单词分类" ></input>
			</view>
        ...
        <view style="display: flex;flex-direction: column;">
          <view :style='categoryName===item.dancifenlei?sactiveItem:sitem' class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.dancifenlei?'tabActive':''" @tap="categoryClick(item.dancifenlei)">{{item.dancifenlei}}</view>
        </view>
      </scroll-view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                                                                          <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"0","borderWidth":"4rpx","width":"240rpx","borderStyle":"solid","height":"180rpx"}' mode="aspectFill" :src="product.dancitupian?baseUrl+product.dancitupian.split(',')[0]:''"></image>
                                                                                                              <view class="list-item-body" style="flex: 1;">
            <view :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.dancimingcheng}}</view>
                                                                                                                                                                                              </view>
                                                                                                                                                      <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('danciben','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('danciben','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('danciben','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('danciben','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('dancifenlei', {page:1,limit:100});
            } else {
                res = await this.$api.list('dancifenlei', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,dancifenlei:'全部'})
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.dancimingcheng="";
				this.searchForm.dancifenlei="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.dancifenlei = '%' + this.categoryName + '%'
				}

				if(this.searchForm.dancimingcheng){
					params['dancimingcheng'] = '%' + this.searchForm.dancimingcheng + '%'
				}
				if(this.searchForm.dancifenlei){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`danciben`, params);
                } else {
                    res = await this.$api.list(`danciben`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('danciben', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.dancimingcheng){
					searchForm['dancimingcheng'] = '%' + this.searchForm.dancimingcheng + '%'
				}
				if(this.searchForm.dancifenlei){
					searchForm['dancifenlei'] = '%' + this.searchForm.dancifenlei + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`danciben`, searchForm);
                } else {
                    res = await this.$api.list(`danciben`, searchForm);
                }
				// 如果是第一页数据置空
