<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="单词分类" prop="dancifenlei">
          <el-input v-model="ruleForm.dancifenlei" 
              placeholder="单词分类" clearable  :readonly="ro.dancifenlei"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="单词解析" prop="dancijiexi">
          <el-input v-model="ruleForm.dancijiexi" 
              placeholder="单词解析" clearable  :readonly="ro.dancijiexi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="单词解析" prop="dancijiexi">
              <el-input v-model="ruleForm.dancijiexi" 
                placeholder="单词解析" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.beizhu = this.ruleForm.beizhu.replace(reg,'../../../ssm65k27/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuexiqingdan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuexiqingdanCrossAddOrUpdateFlag = false;
						 this.parent.search();
